...
-- =========================================
-- 1) Add a new column to store valid DATE values
ALTER TABLE CovidDeaths
ADD COLUMN date_fixed DATE;

-- 2) Clean and convert all formats into DATE
UPDATE CovidDeaths
SET date_fixed =
    CASE
        WHEN TRIM(date) = '' OR date = '0000-00-00' THEN NULL

        -- Example: March 10, 2020
        WHEN date LIKE '%,%' THEN STR_TO_DATE(date, '%M %d, %Y')

        -- Example: 24/02/2020  (DAY/MONTH/YEAR)
...
-- 3) Check if new column is correct
SELECT date, date_fixed FROM CovidDeaths LIMIT 20;

-- 4) Drop old VARCHAR date column
ALTER TABLE CovidDeaths
DROP COLUMN date;

-- 5) Rename date_fixed back to date (as DATE type)
ALTER TABLE CovidDeaths
CHANGE date_fixed date DATE;
...
SELECT date FROM CovidDeaths LIMIT 20;
SELECT date FROM CovidVaccinations LIMIT 20;
...
-- Showing TotalDeaths in different parts of the world
